...
export const WorkloadProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [recommendations, setRecommendations] = useState<Recommendation[] | null>(null);

  // Simulated API call for recommendations
  const fetchRecommendations = (request: RecommendationRequest): Promise<Recommendation[]> => {
            ...
            <div className="p-6 space-y-6">
              {recommendations.map((rec) => (
                <div key={rec.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h3 className="text-lg font-medium text-teal-700">{rec.title}</h3>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <ul className="space-y-1 text-sm text-gray-600">
                        <li><span className="font-medium">CPU:</span> {rec.specs.cpu}</li>
                        <li><span className="font-medium">RAM:</span> {rec.specs.ram}</li>
                        <li><span className="font-medium">Storage:</span> {rec.specs.storage}</li>
                        <li><span className="font-medium">Network:</span> {rec.specs.network}</li>
                      </ul>
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-700 mb-2">Cost Estimate</h4>
                      <p className="text-xl font-semibold text-slate-800">{rec.estimatedCost}</p>
